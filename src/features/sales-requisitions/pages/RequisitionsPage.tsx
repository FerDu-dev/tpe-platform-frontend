...
import { loadRequisitions } from '../store/salesRequisitionsSlice';
...
const RequisitionsPage: React.FC = () => {
    ...
    const requisitions = useAppSelector(state => state.salesRequisitions.requisitions);
    ...
    const meta = useAppSelector(state => state.salesRequisitions.meta);
    const filters = useAppSelector(state => state.salesRequisitions.filters);
    const error = useAppSelector(state => state.salesRequisitions.error);
    ...
    const handleActionConfirm = async () => {
        ...
        try {
            ...
            if (actionType === 'PAUSE') {
                await salesRequisitionService.pause(selectedRequisition.id, reason);
                message.success('Requisición pausada correctamente. Los candidatos vinculados han sido liberados.');
            } else {
                await salesRequisitionService.cancel(selectedRequisition.id, reason);
    ...
    const handleReactivate = async () => {
        ...
        setLoadingAction(true);
        try {
            await salesRequisitionService.reactivate(selectedRequisition.id);
