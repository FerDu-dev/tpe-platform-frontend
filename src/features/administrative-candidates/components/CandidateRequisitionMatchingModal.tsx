...
import { AdministrativeRequisition, Candidate, PaginatedResponse } from '../../../types';
import { administrativeRequisitionService } from '../../../services/administrativeRequisitionService';
import { zonesService } from '../../../services/zonesService';
import { useAppSelector } from '../../../app/store';
import { selectPositions, selectCompanies } from '../../../store/masterDataSlice';
...
const CandidateRequisitionMatchingModal: React.FC<CandidateRequisitionMatchingModalProps> = ({
    ...
    const [data, setData] = useState<AdministrativeRequisition[]>([]);
    ...
    const fetchRequisitions = async () => {
        setLoading(true);
        try {
            const res: PaginatedResponse<AdministrativeRequisition> = await administrativeRequisitionService.fetchRequisitions({
                ...
                type: candidate.category === 'INTERNSHIP' ? 'Pasantía' : 'Profesional'
            } as any);
    ...
    const handleAssignClick = (requisition: AdministrativeRequisition) => {
        ...
        const reqStateName = requisition.state?.name || requisition.country || 'Desconocida';
    ...
    const columns = [
        {
            ...
            key: 'department',
            render: (record: AdministrativeRequisition) => (
                <Space direction="vertical" size={0}>
                    <Text strong>{record.department || 'N/A'}</Text>
