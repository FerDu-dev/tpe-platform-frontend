...
import { loadRequisitions } from '../store/adminRequisitionsSlice';
...
const RequisitionsPage: React.FC = () => {
    ...
    const handleOpenEdit = (requisition: AdministrativeRequisition) => {
        setEditingId(requisition.id.toString());
        setIsFormOpen(true);
    };

    const meta = useAppSelector(state => state.adminRequisitions.meta);
    const filters = useAppSelector(state => state.adminRequisitions.filters);
    const error = useAppSelector(state => state.adminRequisitions.error);
    ...
    const handleRowClick = (record: AdministrativeRequisition) => {
    ...
    const handleActionConfirm = async () => {
        ...
        try {
            ...
            if (actionType === 'PAUSE') {
                await administrativeRequisitionService.pause(selectedRequisition.id, reason);
                message.success('Requisición pausada correctamente. Los candidatos vinculados han sido liberados.');
            } else {
                await administrativeRequisitionService.cancel(selectedRequisition.id, reason);
    ...
    const handleReactivate = async () => {
        ...
        setLoadingAction(true);
        try {
            await administrativeRequisitionService.reactivate(selectedRequisition.id);
        ...
        <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
            <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} style={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
                <Row justify="space-between" align="middle" style={{ marginBottom: '24px' }}>
                    ...
                    <Col>
                        <PermissionGuard module="adminRequisitions" action="create">
                            <Button
                                type="primary"
                                icon={<PlusOutlined />}
                ...
                >
                    ...
                    <TabPane tab="Historial de Contrataciones" key="hires">
                        <PermissionGuard module="adminHires">
                            <HiresPage />
                        </PermissionGuard>
                    </TabPane>
                </Tabs>
