...
const CandidateListModal: React.FC<CandidateListModalProps> = ({ 
    ...
    const columns = [
        ...
        {
            title: 'Requisición / Área',
            key: 'requisition',
            render: (_: any, record: Candidate | any) => (
                <div>
                    <Text>{record.requisitionZoneName || record.position || 'N/A'}</Text>
                    <br />
                    <Text type="secondary" style={{ fontSize: '12px' }}>
                        {typeof record.zone === 'object' ? record.zone?.name : (record.zone || record.department || 'N/A')}
                    </Text>
                </div>
            ),
        },
