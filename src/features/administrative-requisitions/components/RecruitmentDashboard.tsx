...
import {
    ...
    setFilters
} from '../store/adminRequisitionsSlice';
...
import CandidateDrawer from '../../sales-candidates/components/CandidateDrawer';
...
const RecruitmentDashboard: React.FC = () => {
    ...
    useEffect(() => {
        if (filters.jobRequisitionId) {
            setLoadingOps(true);
            administrativeRequisitionService.findOne(filters.jobRequisitionId.toString())
    ...
    const handleRefreshData = () => {
        ...
        if (filters.jobRequisitionId) {
            administrativeRequisitionService.findOne(filters.jobRequisitionId.toString()).then(setFullRequisition);
        }
    };
        ...
        <div style={{ padding: '24px', background: '#f0f2f5', minHeight: '100%' }}>
            ...
            {(!filters.companyId) ? (
                    ...
                    <Space direction="vertical" size="large" style={{ width: '100%' }}>
                        ...
                        >
                            {(() => {
                                ...
                                const grouped = {
                                    A: paginatedReqs.filter((r: any) => r.priority === 'A'),
                                    B: paginatedReqs.filter((r: any) => r.priority === 'B'),
                                    C: paginatedReqs.filter((r: any) => r.priority === 'C'),
                                };
                                                            ...
                                                            <Col xs={24} sm={12} lg={8} key={req.id}>
                                                                ...
                                                                >
                                                                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                                                        <Title level={5} style={{ fontSize: '14px', margin: 0 }}>{req.position}</Title>
                                                                        <Tag color={req.priority === 'A' ? 'error' : 'blue'}>{req.priority}</Tag>
                                                                    </div>
                                                                    <Text type="secondary" style={{ fontSize: '12px' }}>
                                                                        <GlobalOutlined /> {req.department || 'N/A'} - {req.type || 'N/A'}
                                                                    </Text>
                ...
                <Space direction="vertical" size="large" style={{ width: '100%' }}>
                    <Card style={{ ...glassStyle, marginBottom: -16 }} bodyStyle={{ padding: '12px 24px' }}>
                        <Row justify="space-between" align="middle">
                            <Col style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                                ...
                                <Title level={4} style={{ margin: 0 }}>{fullRequisition?.position}</Title>
                                <Tag color="blue">{fullRequisition?.department || 'N/A'}</Tag>
                            </Col>
                        </Row>
                    </Card>
                    ...
                    <Card
                        title={fullRequisition ? `Gestión de Candidatos - ${fullRequisition.position}` : 'Gestión de Candidatos'}
                        style={glassStyle}
                    >
