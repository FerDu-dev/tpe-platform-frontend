...
const SmartMatchingModal: React.FC<SmartMatchingModalProps> = ({ visible, onClose, requisition, onCandidateLinked }) => {
    ...
    const fetchAvailable = async () => {
        setLoading(true);
        try {
            // Determine category based on requisition type
            const category = requisition?.type === 'Pasantía' ? 'INTERNSHIP' : 'PROFESSIONAL';

            // Fetch active candidates of the matching category
            const res = await adminCandidateService.fetchCandidates({ limit: 100, category });
            ...
            const withoutReq = (res.data || []).filter((c: any) => {
                ...
                return !activeApp || !activeApp.administrativeRequisitionId;
            });
    ...
    const handleLink = async (e: React.MouseEvent, candidateId: number) => {
        ...
        try {
            await adminCandidateService.updateApplicationRequisition(candidateId.toString(), requisition.id);
                            ...
                            <Space direction="vertical" size={4} style={{ width: '100%' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                                    <Tag color="blue" style={{ margin: 0, borderRadius: '4px', fontWeight: 600 }}>
                                        <GlobalOutlined /> {requisition?.department || 'N/A'} - {requisition?.type || 'N/A'}
                                    </Tag>
                                    {(requisition?.state?.name) && (
                                        <Tag color="orange" style={{ borderRadius: '4px' }}>Estado: {requisition?.state?.name}</Tag>
                                    )}
                                </div>
                            </Space>
                        </Col>
