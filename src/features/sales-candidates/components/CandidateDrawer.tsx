...
const CandidateDrawer: React.FC<CandidateDrawerProps> = ({ open, onClose, candidate, requisition, onActionComplete, onUpdate: _onUpdate }) => {
    ...
    const targetRequisition = requisition || currentApp?.salesRequisition;
    ...
    const handleAdvance = () => {
        ...
        /* if (!currentApp?.salesRequisitionId) {
        ...
        /* if (currentApp?.salesRequisitionId && currentApp?.salesRequisition?.status !== 'OPEN') {
            const statusLabel = currentApp?.salesRequisition?.status === 'PAUSED' ? 'Pausada' :
                currentApp?.salesRequisition?.status === 'CANCELLED' ? 'Cancelada' : 'Cerrada';
    ...
    const handleHire = () => {
        const targetRequisition = requisition || activeCandidate.applications?.[0]?.salesRequisition;
